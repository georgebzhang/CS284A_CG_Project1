...
void DrawRend::init() {
  ...
  // default pixel sample method and level sample method
  psm = P_NEAREST;
  lsm = L_ZERO;
}
...
void DrawRend::resize( size_t w, size_t h ) {
  ...
  // 4 > 3 for rgb channels? 3 also works?
...
float DrawRend::dot_prod(float& x, float& y, float& x0, float& y0, float& x1, float& y1) {
	return -(x - x0)*(y1 - y0) + (y - y0)*(x1 - x0);
}

bool DrawRend::point_in_tri(float& x, float& y, float& x0, float& y0, float& x1, float& y1, float& x2, float& y2) {
	float dp1, dp2, dp3;

	dp1 = dot_prod(x, y, x0, y0, x1, y1);
	dp2 = dot_prod(x, y, x1, y1, x2, y2);
	dp3 = dot_prod(x, y, x2, y2, x0, y0);

	// return true if (x,y) is bounded by (x0,y0), (x1,y1), and (x2,y2) regardless of
	// counterclockwise or clockwise direction
	return ((dp1 > 0) && (dp2 > 0) && (dp3 > 0)) || !((dp1 > 0) || (dp2 > 0) || (dp3 > 0));
}

bool DrawRend::bary_point_in_tri(double a, double b, double y) {
	return (!(a < 0)) && (!(b < 0)) && (!(y < 0));
}

bool DrawRend::bary_point_in_tri(Vector3D bc) {
	return (!(bc[0] < 0)) && (!(bc[1] < 0)) && (!(bc[2] < 0));
}

Vector3D DrawRend::bary_coords(float cx, float cy, float x0, float y0,float x1, float y1,float x2, float y2) {
	// Barycentric coordinates alpha (a), beta (b), and gamma (g)
	double a, b, g;
	a = (-(cx - x1)*(y2 - y1) + (cy - y1)*(x2 - x1)) / (-(x0 - x1)*(y2 - y1) + (y0 - y1)*(x2 - x1));
	b = (-(cx - x2)*(y0 - y2) + (cy - y2)*(x0 - x2)) / (-(x1 - x2)*(y0 - y2) + (y1 - y2)*(x0 - x2));
	g = 1 - a - b;
	return Vector3D(a, b, g);
}
...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  //             samplebuffer[row][column].fill_pixel(color);
	...
	//for (int x = 0; x < width; ++x) {
	//	for (int y = 0; y < height; ++y) {
	//		// add 0.5f to x and y to test the center of each pixel
	//		float cx = x + 0.5f;
	//		float cy = y + 0.5f;
	//		if (point_in_tri(cx, cy, x0, y0, x1, y1, x2, y2)) {
	//			samplebuffer[y][x].fill_pixel(color);
	//		}
	//	}
	//}

	float xlist[] = { x0, x1, x2 };
	float ylist[] = { y0, y1, y2 };
	float xmax, xmin, ymax, ymin;
	xmax = *max_element(xlist, xlist + 3);
	xmin = *min_element(xlist, xlist + 3);
	ymax = *max_element(ylist, ylist + 3);
	ymin = *min_element(ylist, ylist + 3);
  ...
  //         You also need to implement get_pixel_color() function to support supersampling.
	// samples per side sps of a pixel is sqrt(sample_rate)

	//int sps = sqrt(sample_rate);
	//for (int x = xmin; x < xmax; ++x) {
	//	for (int y = ymin; y < ymax; ++y) {
	//		for (int i = 0; i < sps; ++i) {
	//			for (int j = 0; j < sps; ++j) {
	//				// make sure the components of the calculations below are float or else
	//				// you will lose precision
	//				float cx = x + (float) i/sps + 0.5f/sps;
	//				float cy = y + (float) j/sps + 0.5f/sps;
	//				if (point_in_tri(cx, cy, x0, y0, x1, y1, x2, y2)) {
	//					samplebuffer[y][x].fill_color(j, i, color);
	//				}
	//			}
	//		}
	//	}
  ...
  // Part 4: Add barycentric coordinates and use tri->color for shading when available.
	...
	//					Vector3D bc = bary_coords(cx, cy, x0, y0, x1, y1, x2, y2);
	//					if (bary_point_in_tri(bc)) {
	//						samplebuffer[y][x].fill_color(j, i, tri->color(bc));
	//					}
	//				}
	//			}
	//		}
  ...
  // Part 5: Fill in the SampleParams struct and pass it to the tri->color function.
	...
	//						SampleParams sp;
	//						sp.psm = psm;
	//						sp.lsm = lsm; // lsm = L_ZERO for Part 5
	//						samplebuffer[y][x].fill_color(j, i, tri->color(bc, Vector3D(0, 0, 0), Vector3D(0, 0, 0), sp));
	//					}
	//				}
	//			}
	//		}
			...
			for (int y = ymin; y < ymax; ++y) {
				for (int i = 0; i < sps; ++i) {
					for (int j = 0; j < sps; ++j) {
						...
						Vector3D bc = bary_coords(cx, cy, x0, y0, x1, y1, x2, y2);
						if (bary_point_in_tri(bc)) {
							Vector3D p_dx_bary, p_dy_bary;
							p_dx_bary = bary_coords(cx + 1, cy, x0, y0, x1, y1, x2, y2);
							p_dy_bary = bary_coords(cx, cy + 1, x0, y0, x1, y1, x2, y2);
							SampleParams sp;
							sp.psm = psm;
							sp.lsm = lsm; // lsm = L_ZERO for Part 5
							samplebuffer[y][x].fill_color(j, i, tri->color(bc, p_dx_bary, p_dy_bary, sp));
						}
					}
				}
			}
