...
Color ColorTri::color(Vector3D p_bary, Vector3D p_dx_bary, Vector3D p_dy_bary, SampleParams sp) {
  // Part 4: Fill this in.
	...
	r = p_bary[0] * p0_col[0] + p_bary[1] * p1_col[0] + p_bary[2] * p2_col[0];
	g = p_bary[0] * p0_col[1] + p_bary[1] * p1_col[1] + p_bary[2] * p2_col[1];
	b = p_bary[0] * p0_col[2] + p_bary[1] * p1_col[2] + p_bary[2] * p2_col[2];
...
Color TexTri::color(Vector3D p_bary, Vector3D p_dx_bary, Vector3D p_dy_bary, SampleParams sp) {
  ...
  // Part 6: Fill this in with trilinear sampling as well.
	...
	double u, v;
	u = p_bary[0] * p0_uv[0] + p_bary[1] * p1_uv[0] + p_bary[2] * p2_uv[0];
	v = p_bary[0] * p0_uv[1] + p_bary[1] * p1_uv[1] + p_bary[2] * p2_uv[1];
	sp.p_uv = Vector2D(u, v);

	double p_dx_u, p_dx_v, p_dy_u, p_dy_v;
	p_dx_u = p_dx_bary[0] * p0_uv[0] + p_dx_bary[1] * p1_uv[0] + p_dx_bary[2] * p2_uv[0];
	p_dx_v = p_dx_bary[0] * p0_uv[1] + p_dx_bary[1] * p1_uv[1] + p_dx_bary[2] * p2_uv[1];
	p_dy_u = p_dy_bary[0] * p0_uv[0] + p_dy_bary[1] * p1_uv[0] + p_dy_bary[2] * p2_uv[0];
	p_dy_v = p_dy_bary[0] * p0_uv[1] + p_dy_bary[1] * p1_uv[1] + p_dy_bary[2] * p2_uv[1];
	sp.p_dx_uv = Vector2D(p_dx_u, p_dx_v);
	sp.p_dy_uv = Vector2D(p_dy_u, p_dy_v);
